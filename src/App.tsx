...
const featureHighlights = [
  {
    ...
    description: 'Launch passwordless flows with WebAuthn and platform biometrics in minutes.'
  },
  {
    title: 'Edge Authorisation',
    description: 'Latency under 80ms worldwide thanks to our 38 region PoPs and smart caching.'
  },
  {
    title: 'Brand-ready UI kit',
    description: 'Craft fully themed auth surfaces using our composable React + Tailwind primitives.'
  },
  {
    title: 'Adaptive security',
    description: 'Machine learning powered anomaly detection, MFA escalation, and audit trails.'
  }
]
...
const securityBreakdown = [
  { label: 'Passkeys enrolled', value: '68%', color: 'from-indigo-500 to-purple-500' },
  { label: 'MFA enabled', value: '87%', color: 'from-emerald-400 to-lime-400' },
  { label: 'WebAuthn-ready devices', value: '74%', color: 'from-sky-400 to-cyan-400' },
]

const trustBadges = ['SOC 2 Type II', 'GDPR compliant', 'ISO 27001', 'HIPAA ready']
    ...
    <div className="relative min-h-screen overflow-hidden bg-slate-950 text-white">
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        <footer className="mx-auto w-full max-w-6xl px-6 pb-10 text-center text-sm text-slate-400">
          &copy; 2024 Aurora Identity — Built for teams who obsess over craft and security.
        </footer>
      </div>

      <LoginOverlay show={showLogin} onClose={closeLogin} onSuccess={handleLoginSuccess} />
    </div>
  )
}

function Header({ view, onShowLogin, onLogout }: { view: View; onShowLogin: () => void; onLogout: () => void }) {
  return (
    <header className="mx-auto w-full max-w-6xl px-6 pt-8" style={{ animation: 'slide-down 0.6s ease-out' }}>
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 px-6 py-4 backdrop-blur-xl shadow-lg shadow-indigo-900/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-indigo-300">Aurora</p>
...
function LandingContent({ onShowLogin }: { onShowLogin: () => void }) {
  return (
    <div className="flex h-full flex-col gap-12" style={{ animation: 'fade-in 0.5s ease-out' }}>
      <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <p className="text-lg text-slate-300 sm:text-xl">
              Ship delightful login experiences with enterprise-grade security baked in. Tailor every pixel,
              integrate effortlessly, and scale to billions of requests.
            </p>
          </div>
        ...
        <div className="relative">
          <div className="absolute -inset-4 bg-gradient-to-br from-indigo-500/30 via-purple-500/20 to-sky-500/20 blur-3xl" aria-hidden="true"></div>
          <div className="relative space-y-5 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-2xl">
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-indigo-500/40 to-purple-500/40 p-5 shadow-xl">
              <p className="text-sm uppercase tracking-[0.3em] text-slate-200">Next-gen stack</p>
              <p className="mt-2 text-lg text-white/90">Edge auth, adaptive MFA, real-time alerts</p>
              <div className="mt-4 flex flex-wrap gap-3 text-xs text-white/80">
                <span className="rounded-full border border-white/30 px-3 py-1">Passkeys</span>
                <span className="rounded-full border border-white/30 px-3 py-1">SSO</span>
                <span className="rounded-full border border-white/30 px-3 py-1">Audit trail</span>
                <span className="rounded-full border border-white/30 px-3 py-1">Flows API</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {featureHighlights.map((feature) => (
          <div key={feature.title} className="rounded-3xl border border-white/10 bg-white/5 p-6 text-left backdrop-blur-xl transition hover:border-white/20">
            <p className="text-sm uppercase tracking-[0.3em] text-indigo-200">Feature</p>
      ...
      <div className="flex flex-wrap gap-6 text-sm text-slate-400">
        {trustBadges.map((badge) => (
          <span key={badge} className="rounded-full border border-white/10 px-4 py-2">
            {badge}
          </span>
        ))}
      </div>
...
function DashboardContent({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col gap-8" style={{ animation: 'fade-in 0.6s ease-out' }}>
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/5 px-6 py-6 shadow-2xl shadow-indigo-900/30">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-indigo-300">Control center</p>
          <h2 className="mt-2 text-3xl font-bold text-white">Great to see you again</h2>
          <p className="text-sm text-slate-300">Here&apos;s the live pulse from the last 24 hours.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {dashboardStats.map((stat) => (
          <div key={stat.label} className="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-xl backdrop-blur">
            <p className="text-sm text-slate-300">{stat.label}</p>
            <p className="mt-2 text-3xl font-semibold text-white">{stat.value}</p>
            <p className={`text-sm font-semibold ${stat.positive ? 'text-emerald-300' : 'text-rose-300'}`}>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-white">Realtime activity</h3>
            <span className="text-xs uppercase tracking-[0.3em] text-slate-400">Live</span>
          </div>
          <div className="mt-5 space-y-4">
            {activityFeed.map((activity) => (
              <div key={activity.user} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            {tasks.map((task) => (
              <li key={task.id}>
                <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200 transition hover:border-white/20">
                  ...
                  />
                  <span className={task.completed ? 'line-through text-slate-500' : ''}>{task.label}</span>
                </label>
              </li>
            ))}
          </ul>
        ...
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-indigo-900/60 p-6 shadow-2xl">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-white">Latency heatmap</h3>
          ...
          <div className="mt-6 grid grid-cols-4 gap-3 text-center text-xs">
            {Array.from({ length: 12 }).map((_, index) => (
              <div key={index} className="space-y-2">
                ...
                <p className="text-[0.65rem] text-slate-400">Region {index + 1}</p>
              </div>
            ))}
          </div>
          <p className="mt-4 text-xs text-slate-400">Median response 46ms • 0.02% error budget used</p>
        </div>
      </div>
    </div>
  )
...
function LoginOverlay(
  { show, onClose, onSuccess }: { show: boolean; onClose: () => void; onSuccess: () => void }
) {
  return (
    <div
    ...
    >
      ...
      <div className={`relative z-10 w-full max-w-5xl transition-all duration-500 ${
